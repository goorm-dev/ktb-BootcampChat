...
const { redisHost, redisPort, redisCluster } = require('../config/keys');

/**
 * MockRedisClient: 실제 Redis 서버가 없을 때 동작하는 인메모리 대체 구현
 */
...
class RedisClient {
  ...
  async connect() {
    ...
    if (!redisHost || !redisPort) {
      console.warn('[Redis] No redisHost or redisPort found, using MockRedisClient');
    ...
    try {
      if (process.env.NODE_ENV === 'production' || redisCluster === 'true') {
        const hosts = redisHost.split(',').map((h, i) => ({
          host: h.trim(),
          port: parseInt((redisPort + '').split(',')[i] || redisPort, 10)
        }));
  
        console.log(`[Redis] Connecting in CLUSTER mode to:`, hosts);
  
        this.client = new IORedis.Cluster(hosts, {
      ...
      this.client.on('error', (err) => {
        ...
        if (!this.useMock) {
          console.warn('[Redis] Falling back to MockRedisClient');
          this.client = new MockRedisClient();
          this.isConnected = true;
          this.useMock = true;
        }
      ...
      await this.client.ping(); // ioredis는 connect 메서드 없음
  ...
  async set(key, value, options = {}) {
    ...
    const stringValue = typeof value === 'object' ? JSON.stringify(value) : String(value);
    if (options.ttl) return await this.client.set(key, stringValue, 'EX', options.ttl);
  ...
  async setEx(key, seconds, value) {
    ...
    const stringValue = typeof value === 'object' ? JSON.stringify(value) : String(value);
    return await this.client.set(key, stringValue, 'EX', seconds);
  }
  ...
  async lPush(key, value) {
    if (!this.isConnected) await this.connect();
    if (this.useMock) return this.client.lPush(key, value);
    return await this.client.lpush(key, value);
  }

  async lTrim(key, start, stop) {
    if (!this.isConnected) await this.connect();
    if (this.useMock) return this.client.lTrim(key, start, stop);
...
// 환경변수 예시
// 운영 클러스터: REDIS_HOST=10.0.0.1,10.0.0.2,10.0.0.3 REDIS_PORT=6379,6379,6379 REDIS_CLUSTER=true
