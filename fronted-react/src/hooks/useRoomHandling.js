...
export const useRoomHandling = (
  ...
  const handleSessionError = async () => {
    ...
    if (mountedRef.current) {
      ...
      navigate(`/?redirect=${encodeURIComponent(redirectPath)}`, { replace: true });
    }
    return false;
  };
  ...
  const setupSocket = useCallback(async () => {
    try {
      ...
      if (error.message === 'Invalid authentication state') {
         navigate('/?error=auth_required', { replace: true });
      }
      throw error;
    }
  }, [userRooms, cleanup, navigate]);
  ...
  const fetchRoomData = useCallback(async (currentRoomId) => {
    try {
      ...
      if (!currentRoomId || !mountedRef.current) {
        throw new Error('채팅방 정보가 올바르지 않습니다.');
      }

      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/rooms/${currentRoomId}`,
        {
          method: 'GET',
          headers: {
